...
module lpt_class
   ...
   integer           , dimension(part_nblock) :: part_lblock=[1,35,4]
   ...
   contains
      ...
      procedure :: get_nondim_properties                 !< Compute non-dimensional properties
...
contains
   ...
   subroutine advance(this,dt,U,V,W,rho,visc,stress_x,stress_y,stress_z,vortx,vorty,vortz,T,srcU,srcV,srcW,srcE,tdevu,tdevv,tdevw)
      ...
      integer :: i,j,k,ierr
      real(WP) :: mydt,dt_done,deng,Ip,frho
      ...
      do i=1,this%np_
         ...
         block
            real(WP) :: fs, fv
            real(WP) :: massp,  tmpdt
            real(WP), dimension(3) ::cur_torque

            massp = this%rho*Pi/6.0_WP*myp%d**3

            call this%get_rhs(U=U,V=V,W=W,rho=rho,visc=visc,stress_x=sx,stress_y=sy,stress_z=sz,vort_x=vortx,vort_y=vorty,vort_z=vortz,&
               tdevu=tdevu,tdevv=tdevv,tdevw=tdevw,p=myp,acc=acc,torque=cur_torque,opt_dt=tmpdt)
            ...
            myp%Torque = cur_torque*massp
            myp%torqueCoeff = (2.0_WP*norm2(myp%Torque)) / (frho*norm2(myp%angVel-0.5_WP*fvort)*(0.5_WP*myp%d)**5.0_WP)
         end block
   ...
   subroutine get_rhs(this,U,V,W,rho,visc,stress_x,stress_y,stress_z,vort_x,vort_y,vort_z,tdevu,tdevv,tdevw, T,p,acc,torque,opt_dt)
      ...
      compute_lift: block
         use mathtools, only: Pi,cross_product, normalize
         real(WP) :: J, Clv, Cls
         real(WP), dimension(3) :: Flift, dirv, dirs

         ! Compute acceleration due to vorticity induced lift
         if (p%Re_p .le. 50.0_WP) then
            ! McLaughlin lift
            J = 0.3*(1+tanh(2.5_WP*(log10(sqrt(p%nondimOmega_F/p%Re_p))+0.191_WP)))*(2.0_WP/3.0_WP+tanh(6.0_WP*(sqrt(p%nondimOmega_F/p%Re_p))-1.92_WP))
            Clv = 0.1714_WP*sqrt(p%Re_p*p%nondimOmega_F)*Cd
            Clv = Clv*J
         else
            ! Loth 2008
            Clv = -1.0_WP*p%nondimOmega_F**(1.0_WP/3.0_WP)*(0.0525+0.0575*tanh(5.0_WP*log10(p%Re_p/120.0_WP)))
         end if

         ! Tanaka 1990; Tri 1990; B&B 2002; Loth 2008
         Cls = 1-(0.675_WP + 0.15_WP*(1.0_WP+tanh(0.28_WP*(p%nondimOmega_P-2.0_WP))))*tanh(0.18_WP*sqrt(p%Re_p))

         ! Compute acceleration due to spinning induced lift
         dirv = normalize(cross_product(fvort,(fvel-p%vel)))
         dirs = normalize(cross_product(p%angVel,(fvel-p%vel)))
         Flift = (Clv*dirv+Cls*dirs)*(Pi/8.0_WP*frho*sum((p%vel-fvel)**2)*p%d**2)
         acc = acc + Flift/(this%rho*Pi/6.0_WP*p%d**3)
      ...
      compute_torque: block
         ...
         ! Salem and Osterle 1998
         corrV=1.0_WP
         ! Sommerfeld 2001
         corrS=1.0_WP+0.012_WP*p%Re_omega**0.7_WP
   ...
   !> Calculate the non-dimensional particle properties
   subroutine get_nondim_properties(this,U,V,W,rho,visc,vort_x,vort_y,vort_z,p)
      implicit none
      class(lpt), intent(inout) :: this
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: U         !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: V         !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
   ...
   end subroutine get_nondim_properties
