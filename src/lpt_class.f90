...
module lpt_class
  ...
  type :: part
     ...
     real(WP), dimension(6) :: SR          !< local Strain rate tensor
     !> MPI_INTEGER data
...
contains
  ...
  subroutine advance(this,dt,U,V,W,rho,visc,stress_x,stress_y,stress_z,vortx,vorty,vortz,T,srcU,srcV,srcW,srcE,tdevu,tdevv,tdevw)
    ...
    integer :: i,j,k,ierr
    real(WP) :: mydt,dt_done,deng,Ip,frho,fvisc
    ...
    do i=1,this%np_
       ...
       do while (dt_done.lt.dt)
          ...
          call this%get_rhs(U=U,V=V,W=W,rho=rho,visc=visc,stress_x=sx,stress_y=sy,stress_z=sz,&
       ...
       ! Get non-dimensional features
       fvel=this%cfg%get_velocity(pos=myp%pos,i0=myp%ind(1),j0=myp%ind(2),k0=myp%ind(3),U=U,V=V,W=W)
       fvisc=this%cfg%get_scalar(pos=myp%pos,i0=myp%ind(1),j0=myp%ind(2),k0=myp%ind(3),S=visc,bc='n')
       frho=this%cfg%get_scalar(pos=myp%pos,i0=myp%ind(1),j0=myp%ind(2),k0=myp%ind(3),S=rho,bc='n')
       myp%Re_p = frho*norm2(myp%vel-fvel)*myp%d/(fvisc+epsilon(1.0_WP))
  ...
  subroutine get_rhs(this,U,V,W,rho,visc,stress_x,stress_y,stress_z,tdevu,tdevv,tdevw,T,p,acc,torque,opt_dt)
    implicit none
    class(lpt), intent(inout) :: this
    real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: U         !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
    real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: V         !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
