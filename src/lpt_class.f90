...
module lpt_class
  ...
  type :: part
     ...
     real(WP), dimension(3) :: Tshear     !< Shear stress torque
     real(WP) :: dt                       !< Time step size for the particle
     real(WP) :: Re                       !< Reynolds number of particle
     real(WP) :: St                      !< Stokes number of particle
     !> MPI_INTEGER data
  ...
  integer           , dimension(part_nblock) :: part_lblock=[1,22,4]
...
contains
  ...
  subroutine advance(this,dt,U,V,W,rho,visc,stress_x,stress_y,stress_z,vortx,vorty,vortz,T,srcU,srcV,srcW,srcE)
    ...
    do i=1,this%np_
       ...
       dt_done=0.0_WP
       call this%get_rhs(U=U,V=V,W=W,rho=rho,visc=visc,stress_x=sx,stress_y=sy,stress_z=sz,vort_x=vortx,vort_y=vorty,vort_z=vortz,p=myp,acc=acc,torque=torque,opt_dt=myp%dt,Re_o=Re)
       myp%Tshear = torque
       myp%Re = Re
       fvel=this%cfg%get_velocity(pos=myp%pos,i0=myp%ind(1),j0=myp%ind(2),k0=myp%ind(3),U=U,V=V,W=W)
       myp%St = this%rho*norm2(myp%vel-fvel)*myp%d/visc(myp%ind(1),myp%ind(2),myp%ind(3))/18.0_WP
  ...
  subroutine get_rhs(this,U,V,W,rho,visc,stress_x,stress_y,stress_z,vort_x,vort_y,vort_z, T,p,acc,torque,opt_dt,Re_o)
    implicit none
    class(lpt), intent(inout) :: this
    real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: U         !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
    real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: V         !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
